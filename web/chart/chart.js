class Chart{
   ...
   #drawSamples(samples) {
      ...
      for (const sample of samples) {
         ...
         // Check if this.styles[label] is defined and has a 'color' property
         if (this.styles[label] && this.styles[label].color) {
            // Draw point with color if available
            graphics.drawPoint(ctx, pixelLoc, this.styles[label].color);
         } else if (this.styles[label] && this.styles[label].text) {
            // Fall back to drawing image if 'image' property is available
